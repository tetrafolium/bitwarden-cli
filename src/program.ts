...
export class Program extends BaseProgram {
    ...
    run() {
        program
            ...
            .version(
                this.main.platformUtilsService.getApplicationVersion(),
                "-v, --version"
            );
        ...
        program.on("command:*", () => {
            writeLn(
                chalk.redBright("Invalid command: " + program.args.join(" ")),
                false,
                true
            );
        ...
        program
            ...
            .option(
                "--url <url>",
                "Filter items of type login with a url-match search."
            )
        ...
        program
            ...
            .on("--help", () => {
                ...
                writeLn("");
                writeLn(
                    "    `encodedJson` can also be piped into stdin. `encodedJson` contains " +
        ...
        program
            ...
            .on("--help", () => {
                ...
                writeLn("");
                writeLn(
                    "    Valid formats are `csv` and `json`. Default format is `csv`."
                );
                writeLn("");
                writeLn("  Examples:");
                writeLn("");
    ...
    protected processResponse(response: Response, exitImmediately = false) {
        super.processResponse(response, exitImmediately, () => {
            if (response.data.object === "template") {
                return this.getJson(
                    (response.data as TemplateResponse).template
                );
            }
            return null;
        });
    ...
    private async exitIfLocked() {
        ...
        if (!hasKey) {
            const canInteract =
                process.stdout.isTTY && process.env.BW_NOINTERACTION !== "true";
            if (canInteract) {
                const command = new UnlockCommand(
                    this.main.cryptoService,
                    this.main.userService,
