        ...
        try {
            ...
            const groups =
                req.groups == null
                    ? null
                    : req.groups.map(
                          g => new SelectionReadOnlyRequest(g.id, g.readOnly)
                      );
            const request = new CollectionRequest();
            request.name = (
                await this.cryptoService.encrypt(req.name, orgKey)
            ).encryptedString;
            request.externalId = req.externalId;
            request.groups = groups;
            await this.apiService.putCollection(
                req.organizationId,
                id,
                request
            );
            const res = new OrganizationCollectionResponse(
                Collection.toView(req),
                groups
            );
            return Response.success(res);
        } catch (e) {
