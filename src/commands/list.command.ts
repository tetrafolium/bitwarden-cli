...
export class ListCommand {
        ...
        if (
            ...
            ciphers = ciphers.filter(c => {
                if (cmd.folderid != null) {
                    ...
                    const folderId =
                        cmd.folderid === "null" ? null : cmd.folderid;
                ...
                if (cmd.collectionid != null) {
                    ...
                    const collectionId =
                        cmd.collectionid === "null" ? null : cmd.collectionid;
                    if (
                        collectionId == null &&
                        (c.collectionIds == null ||
                            c.collectionIds.length === 0)
    ...
    private async listCollections(cmd: program.Command) {
        ...
        const res = new ListResponse(
            collections.map(o => new CollectionResponse(o))
        );
        return Response.success(res);
    }
    ...
    private async listOrganizationCollections(cmd: program.Command) {
        ...
        try {
            ...
            if (organization.isAdmin) {
                response = await this.apiService.getCollections(
                    cmd.organizationid
                );
            } else {
            ...
            const collections = response.data
                .filter(c => c.organizationId === cmd.organizationid)
                .map(
                    r =>
                        new Collection(
                            new CollectionData(
                                r as ApiCollectionDetailsResponse
                            )
                        )
                );
            let decCollections = await this.collectionService.decryptMany(
                collections
            );
            if (cmd.search != null && cmd.search.trim() !== "") {
                decCollections = CliUtils.searchCollections(
                    decCollections,
                    cmd.search
                );
            }
            const res = new ListResponse(
                decCollections.map(o => new CollectionResponse(o))
            );
            return Response.success(res);
        } catch (e) {
    ...
    private async listOrganizationMembers(cmd: program.Command) {
        ...
        try {
            const response = await this.apiService.getOrganizationUsers(
                cmd.organizationid
            );
            const res = new ListResponse(
                response.data.map(r => {
    ...
    private async listOrganizations(cmd: program.Command) {
        ...
        if (cmd.search != null && cmd.search.trim() !== "") {
            organizations = CliUtils.searchOrganizations(
                organizations,
                cmd.search
            );
        }
        ...
        const res = new ListResponse(
            organizations.map(o => new OrganizationResponse(o))
        );
        return Response.success(res);
    }
}
